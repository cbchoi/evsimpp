...
evsim::CHierarchicalCoupled::CHierarchicalCoupled(Model pbehavior, CExecutorFactory* ef, CModel* engine, Time creation_t,  Time current_t)
	:bobject(pbehavior), request_t(current_t), next_event_t(current_t), m_engine(engine)
{
	...
	{
		IExecutor executor = ef->create_entity(pairs.second.get(), engine, creation_t, current_t);
...
void evsim::CHierarchicalCoupled::external_transition(const port& _port, MessageDeliverer& deliver)
{
	for (auto& msg : deliver.get_contents())
	{
		...
		if (iter != behavior_object->get_couplings().end())
		{
			for (coupling_relation cr : iter->second)
			{
				...
				if (dst != m_model_executor_map.end())
				{
					...
					dst->second->external_transition(*cr.port, deliver);
